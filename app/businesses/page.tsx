export default function Businesses() {
  return (
    <div className="min-h-screen bg-white">
      <section className="pt-32 pb-16 px-6 sm:px-8 lg:px-12">
        <div className="max-w-7xl mx-auto">
          <div className="max-w-4xl">
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-medium text-neutral-900 mb-8 leading-tight tracking-tight">
              Our Businesses
            </h1>
            <p className="text-xl sm:text-2xl text-neutral-600 leading-relaxed">
              A portfolio of quality businesses built for long-term value creation.
            </p>
          </div>
        </div>
      </section>

      <section className="py-16 px-6 sm:px-8 lg:px-12">
        <div className="max-w-5xl mx-auto">
          <div className="border-l-2 border-neutral-900 pl-6 mb-12">
            <h2 className="text-2xl font-medium text-neutral-900 mb-2">Portfolio Companies</h2>
            <p className="text-sm text-neutral-600">Currently under development</p>
          </div>
          <div className="bg-neutral-50 rounded-lg p-12 text-center">
            <p className="text-base text-neutral-600">
              Information about our portfolio companies will be available soon.
            </p>
          </div>
        </div>
      </section>
    </div>
  );
}
